        ...
        if identifier then
            if ESX.GetPlayerFromIdentifier(identifier) then
                ...
                if not checkStatus then 
                    return deferrals.done("[Jay] You are not allowed to join this server. Please contact the server owner for more information. discord.gg/fC5AwJjSJ4")
                else 
                    return deferrals.done()
                end
            end
...
function loadESXPlayer(identifier, playerId, isNew)
    ...
    -- xPlayer Creation
    local xPlayer = CreateExtendedPlayer(playerId, identifier, userData.group, userData.accounts, userData.inventory, userData.job, userData.loadout, GetPlayerName(playerId), userData.coords, userData.metadata)
